...
| **RegLineage** | Capability-lease runtime: hash-bound, expiring, narrowly scoped "leases" for AI data access that fail closed when governance context changes; egress firewall; zero-dep MCP server; review→approve→publish→readback workflow | 33.5K src / 16.1K tests | **408 passed** offline (228 unit + 36 capability + monitoring/mcp/integration suites) | Apache-2.0 (copyright line unfilled) | Solo (92 commits; 1 stray first commit by `rasbhalerao-rgb` at repo creation — resolve, see hygiene) | Runtime + firewall + MCP server are real. "Regulatory change" extraction does not exist. |
...
| **`datboiathop` repos** (`records`, `headlines`) | **Not Alex's account.** Owner is Juan Lopez (UPenn; display name `jellywayjuan`, commits by `[email redacted]` / `[email redacted]`) | — | — | none | **Not the principal** | Removed from `assets/`. See ASKS. |
...
**License / collaborators.** Apache-2.0, copyright line unfilled. One stray commit at repo creation by `rasbhalerao-rgb` — likely template/scaffold; **confirm and document before commercializing** (ASK-004). Hackathon/sponsor terms not present in repo — confirm.
...
## 8. `datboiathop` — provenance error

`gh api users/datboiathop` → name `jellywayjuan`, created 2021-09-28; every commit in `records`/`headlines` is `Juan <[email redacted]>`; earliest commit `Juan Lopez <[email redacted]>`; coursework is UPenn CIS 1200 / 3800 / 4120 (fall 2025, concurrent with Alex at Northeastern). **This machine's `gh` CLI is authenticated as this account** (`gh auth status`). Consequences: (a) `records`/`headlines` were removed from `assets/`; (b) no GitHub write action (PRs, issues, comments) may happen from this machine until `gh auth login` is redone as `Alex-lop` (ASK-001); (c) `records` has a **non-expiring Spotify client secret committed in plaintext** in four public files plus Firebase project config — the account owner must rotate it and purge history (ASK-002).
...
4. Confirm the stray `rasbhalerao-rgb` commit in RegLineage and the absence of hackathon/sponsor IP terms for Graphene, RegLineage, Nemisis.
