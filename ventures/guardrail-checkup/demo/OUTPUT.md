...
- **Size:** 9 file(s) considered, 1,926 bytes (apparent size)
...
- Your MCP configuration (`.mcp.json`) was rewritten as a **suggestion** with `egresswall proxy` in front of 2 of 2 server(s). It was not applied; see §4.
...
| --- | --- | --- |
| “Testing: 0/0 (0%)” | 2 file(s) sit in a test path | `git ls-files --cached --others --exclude-standard \| grep -cE '(^\|/)(tests?\|spec)/\|(^\|/)test_[^/]+$\|_test\.[a-z]+$\|\.(test\|spec)\.[jt]sx?$'` |
...
- **Evidence (score 2):**
  - path heuristic: 2 file(s) matching app/checkout.py, tests/test\_checkout.py
...
3. Compare `drafts/mcp-wrapped.json` with your own MCP configuration. It is the same servers with `egresswall proxy` in front of the 2 it could wrap; nothing was applied. Its `--policy` names `/etc/egresswall/policy.json`, a placeholder — point it at an egresswall policy you write, because this tool writes none.
