# Agent guardrail checkup — Nemisis

Run 2026-08-31 · read-only · `Nemisis` · HEAD `a5951ab6b5a8`
...
- **Repository:** `Nemisis`, as given on the command line
- **HEAD:** `a5951ab6b5a8031b16c1ea57dbabcd652022fa06`
- **Size:** 85 file(s) considered, 1,052,354 bytes (apparent size)
...
- **Language mix** (by file extension), the 10 most common of 12:
  ...
  - `.ini` — 1
  - `.lock` — 1
- **Read:** the named guardrail artifacts listed in §2, `pyproject.toml`, `setup.cfg`, `package.json` (for the linter falsifier), every checked-in `.json` file the signature scan reached before its 64 MiB and 10,000 file budgets were spent, and up to 2 JSON fixture(s) screened in §2. A file over 1 MiB, or one whose first 8 KiB contains a NUL byte, is listed and not read.
...
| .github/workflows/ci.yml: a test runner is named in a run: step, runs on pull requests | `.github/workflows/ci.yml:25` | a test runner is named here and this workflow runs on pull requests; whether it ran, and on what, was not checked, because nothing here was executed |
...
| tests: 23 file(s) in a test path | `docs/assets/crashcheck-hero/repros/double-credit/1025d9c6e014394cf80629d180e7cb4fb1a77a4b7b26934980b5f5ea975069a8/test_repro.py:1` | there is a suite a hook or a CI gate can call |
...
- `src/nemisis/fixtures/sqlite_credit_v1/contract.json` — clean under egresswall's default policy
- `src/nemisis/fixtures/sqlite_credit_v1/event.json` — clean under egresswall's default policy
...
| “Missing linter configuration” | a linter is configured at `pyproject.toml:25` | `grep -n -iE 'ruff\|eslint\|biome\|flake8\|prettier' pyproject.toml` |
| “Testing: 0/0 (0%)” | 23 file(s) sit in a test path | `git ls-files --cached --others --exclude-standard \| grep -cE '(^\|/)(tests?\|spec)/\|(^\|/)test_[^/]+$\|_test\.[a-z]+$\|\.(test\|spec)\.[jt]sx?$'` |
...
- **Evidence (score 2):**
  ...
  - git history: 1 repair commit(s) touched these paths — 4bccf78 fix: harden verifier evidence boundaries
...
### Invariant candidate 2 — An agent does not write to secret material without a human deciding first.
...
### Invariant candidate 3 — An agent does not write to deployment and infrastructure without a human deciding first.
...
- **Command:** `guardrail-checkup run Nemisis --out docs/dogfood/Nemisis.md`
- **Repository commit:** `a5951ab6b5a8031b16c1ea57dbabcd652022fa06`
- **Repair commits examined:** 8, from the last 2000 non-merge commits
