# Agent guardrail checkup — Scweet

Run 2026-06-24 · read-only · `Scweet` · HEAD `c42e1222c632`
...
- **Repository:** `Scweet`, as given on the command line
- **HEAD:** `c42e1222c632dbfeb5ae91633f426a6bd44a677a`
- **Size:** 79 file(s) considered, 3,513,831 bytes (apparent size)
...
- **Language mix** (by file extension), the 10 most common of 12:
  ...
  - `.iml` — 1
  - `.in` — 1
- **Read:** the named guardrail artifacts listed in §2, `pyproject.toml`, `setup.cfg`, `package.json` (for the linter falsifier), every checked-in `.json` file the signature scan reached before its 64 MiB and 10,000 file budgets were spent, and up to 1 JSON fixture(s) screened in §2. A file over 1 MiB, or one whose first 8 KiB contains a NUL byte, is listed and not read.
...
| .github/workflows/tests.yml: a test runner is named in a run: step, runs on pull requests | `.github/workflows/tests.yml:28` | a test runner is named here and this workflow runs on pull requests; whether it ran, and on what, was not checked, because nothing here was executed |
...
| tests: 21 file(s) in a test path | `tests/conftest.py:1` | there is a suite a hook or a CI gate can call |
...
- `examples/cookies.json` — **1 violation(s)** under egresswall's default policy:
  - `FORBIDDEN_KEY at response[0].cookies.auth_token: field name 'auth_token' is forbidden by policy`
...
| --- | --- | --- |
| “Testing: 0/0 (0%)” | 21 file(s) sit in a test path | `git ls-files --cached --others --exclude-standard \| grep -cE '(^\|/)(tests?\|spec)/\|(^\|/)test_[^/]+$\|_test\.[a-z]+$\|\.(test\|spec)\.[jt]sx?$'` |
...
Only 1 of the categories this tool knows matched a path here, so it names 1 candidate(s) and not three. The runbook's rule applies: do not invent a third.

All 1 of these is a bare path match at score 1: the path heuristic matched and nothing else did — no repair commit and no CODEOWNERS entry points at them. Equal scores are ordered by the number of matching files, then by name; that ordering is not evidence.
...
- **Command:** `guardrail-checkup run Scweet --out docs/dogfood/Scweet.md`
- **Repository commit:** `c42e1222c632dbfeb5ae91633f426a6bd44a677a`
- **Repair commits examined:** 21, from the last 2000 non-merge commits
