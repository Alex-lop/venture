# Agent guardrail checkup — agent-plan-lint

Run 2026-08-31 · read-only · `agent-plan-lint` · HEAD `7408ee7cb90e`
...
- **Repository:** `agent-plan-lint`, as given on the command line
- **HEAD:** `7408ee7cb90e8552a3a7738919795d23618f5292`
- **Size:** 39 file(s) considered, 444,657 bytes (apparent size)
...
| tests: 10 file(s) in a test path | `tests/conftest.py:1` | there is a suite a hook or a CI gate can call |

### agent-plan-lint

- `demo/policy.json` — loads: a valid agent-plan-lint policy
- `demo/plan-bad.json` — 4 issue(s)
- `demo/plan-good.json` — within policy
  - `demo/plan-bad.json: criterion_model_assertion — a model assertion cannot verify a success criterion`
  - `demo/plan-bad.json: cycle — task dependency graph contains a cycle`
  - `demo/plan-bad.json: parallel_write_conflict — tasks work-api, work-models overlap write scope: app/api.py`
  - `demo/plan-bad.json: write_path_not_allowed — write path is forbidden: docs/guide.md`

### egresswall

- `demo/plan-bad.json` — clean under egresswall's default policy
- `demo/plan-good.json` — clean under egresswall's default policy
...
| “Missing linter configuration” | a linter is configured at `pyproject.toml:71` | `grep -n -iE 'ruff\|eslint\|biome\|flake8\|prettier' pyproject.toml` |
| “Testing: 0/0 (0%)” | 10 file(s) sit in a test path | `git ls-files --cached --others --exclude-standard \| grep -cE '(^\|/)(tests?\|spec)/\|(^\|/)test_[^/]+$\|_test\.[a-z]+$\|\.(test\|spec)\.[jt]sx?$'` |
...
- **Command:** `guardrail-checkup run agent-plan-lint --out dogfood/agent-plan-lint.md`
- **Repository commit:** `7408ee7cb90e8552a3a7738919795d23618f5292`
