...
Run 2026-08-29 · read-only · `Graphene` · HEAD `30c9271a4daa`
...
- **Repository:** `Graphene`, as given on the command line
...
- **Evidence (score 3):**
  ...
  - git history: 2 repair commit(s) touched these paths — 1df0ecb fix(ci): make "Generated files leave a clean diff" able to detect that; 3e9f013 fix(repro): lock ruff, kill the hang class, and stop morning\_verify hiding failures
...
- **Evidence (score 2):**
  ...
  - git history: 1 repair commit(s) touched these paths — 3e9f013 fix(repro): lock ruff, kill the hang class, and stop morning\_verify hiding failures
- **An agent breaks it by:** reading two files nearby, inferring the pattern, and writing the change here rather than asking — which is the correct move everywhere else in this repository.
- **Hook** — `.claude/settings.json`:
...
### Invariant candidate 3 — An agent does not write to generated and vendored files without a human deciding first.
...
- **Command:** `guardrail-checkup run Graphene --out dogfood/Graphene.md`
- **Repository commit:** `30c9271a4daa917de8015a5665b9bc26d7f1a201`
- **Repair commits examined:** 46, from the last 2000 non-merge commits
