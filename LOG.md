...
- **First customer-facing artifact** → `ventures/ma-filing-feed/samples/2026-W36-sample-digest.md`: 320 filings across 30 towns, hand-compiled by 9 collectors with robots.txt respected and homeowners anonymized. Kept as proof of method after B was killed.
...
**Learned**
- `gh` on this machine is authenticated as **`datboiathop` = Juan Lopez (UPenn), not Alex** — no GitHub writes until ASK-001; that account's public repo leaks a Spotify client secret (ASK-002). `AC-Washing-Well@commit-changes` publishes CS2800 coursework (ASK-003).
