...
- **What:** `agent-plan-lint` 0.1.0 is released on GitHub — [https://github.com/Alex-lop/agent-plan-lint](https://github.com/Alex-lop/agent-plan-lint), tag `v0.1.0`, 437 passed / 51 skipped on its pinned 3.11, 488 collected — but not on PyPI: the agent's session declined the publish step (the keychain-token read + `uv publish`), and no other session may route around that decline. From a clone of the repository at `v0.1.0`:
  ...
  `egresswall` 0.1.0 is also released on GitHub — [https://github.com/Alex-lop/egresswall](https://github.com/Alex-lop/egresswall), tag `v0.1.0`, 675 tests on 3.11/3.12/3.13, `mcp-name: io.github.Alex-lop/egresswall` already in its README (the MCP Registry ownership check needs it before the upload): the same one-liner from a clone at `v0.1.0`. `guardrail-checkup` is listed here when its repo and tag go live. The standing approval for PyPI is already yes — the block is the tool, not the policy.
