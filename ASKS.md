...
## ASK-012 — Merge five verified PRs on your own repos (one click each)
- **What:** LICENSE copyright lines — Graphene #12, RegLineage #9, Nemisis #1 (CI green), graphene-site #1 (no CI); deployment images build + `procps` — Graphene #13 `fix/deploy-image-procps` (two adversarial verification passes; both images build and run `process_control` in a container). These are GREEN under CLAUDE.md §2, but the harness's permission layer declined the agent's merge action, so the clicks are yours. Do **not** delete the branches (RED).
