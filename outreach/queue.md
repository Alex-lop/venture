...
**Every count in these drafts is a placeholder until the package is frozen (changed 2026-08-31).** `{PL_TESTS}`, `675` and `49` are filled at release from the frozen README, never typed in by hand here. The drafts previously read "364 tests" for plan-lint and "368 tests, 38 hostile servers" for egresswall and all three were false: while they were being verified the real counts moved 406 → 408 → 411 and 368 → 405 → 413, and the packages' own doc-truth tests were failing on exactly those claims. A wrong test count in a Show HN is the first thing a hostile reader checks. §C row 0b is the gate that fills them.
...
### B.3 MCP Registry publishing checklist — step 1 (the `mcp-name:` README line) is **done** in the released source; steps 2–4 follow the PyPI upload
