...
| 2 ship | RUNNING | **agent-plan-lint**: repo + v0.1.0 tag live (https://github.com/Alex-lop/agent-plan-lint; main 7408ee7 adds a docstring correction after the tag; build from main), nine verification rounds. **egresswall**: repo + v0.1.0 tag live (https://github.com/Alex-lop/egresswall), ten verification rounds, round 10 red team 0/0 and claims 0 false sentences, 675 tests, `mcp-name:` line in README. **PyPI upload for both = ASK-015** (2a's harness declines the token/publish step). **guardrail-checkup**: eight verification rounds done (clean-clone PASS since round 2; 467 tests; round 8: red team 3 blockers — set-less recorder dedup quadratic under the signature scan, per-line costs under the file-counted workflow and rule-file caps — claims 2 precision blockers on the churn cut and the every-row sentence). Round 9 running (wf_7d744d79-7d6): final fix + focused verify. Release if clean; otherwise it carries into session 3 as in-verification — this is the last round this session. Wiring done: Graphene #14 and RegLineage #10 RELEASE-LINKs filled (plain pushes), site pages say source released / index pending, queue §A/§B repo URLs and counts filled; {PYPI_URL} stays a placeholder until the upload |
